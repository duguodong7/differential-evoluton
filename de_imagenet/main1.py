...
parser.add_argument('--data', default='/root/imagenet',
...
parser.add_argument('--popsize', type=int, default=10, metavar='N',
                    help='number of label classes (default: 1000)')
parser.add_argument('--model_load_path', type=str, default='resnet50-0676ba61.pth', metavar='N',
                    help='number of label classes (default: 1000)')
...
# logging.basicConfig(level=logging.DEBUG, filename='/home/guodong/declc/output/sgd_bs64.txt', filemode='a')
...
def main():
    ...
    # torchvision.models.vit_b_32(pretrained=True)    #weights=ViT_B_32_Weights.IMAGENET1K_V1
    # model.load_state_dict(torch.load('/root/declc_guodong/resnet50-0676ba61.pth'))
    # _logger.info('Loading from resnet50-0676ba61.pth')
    model.load_state_dict(torch.load(args.model_load_path))
    _logger.info(f'Loading from ....{args.model_load_path}')
    ...
    dataset_de = ImageDataset(train_dir)
    ...
    if args.local_rank == 0:
        ...
        saver = CheckpointSaver(
            ...
            checkpoint_dir=output_dir, recovery_dir=output_dir, decreasing=decreasing, max_history=300)
    ...
    try:  # train the model
        for epoch in range(start_epoch, num_epochs):
            ...
            if len(population) > args.popsize: population = population[-args.popsize:]
            ...
            # Get_Pop(model=model, popsize=args.popsize)
            # score = score_func(solution)
            # score_lst.append(eval_metrics['top1'])
            ...
            if saver is not None and epoch <= 30:
                # save proper checkpoint with eval metric
                save_metric = eval_metrics[eval_metric]
                # saver.save_checkpoint(epoch, metric=None)
                best_metric, best_epoch = saver.save_checkpoint(epoch, metric=save_metric)
            # '''
        for epoch in range(num_epochs, num_epochs+args.de_epochs):
            if args.distributed:
...
def DE_epoch(epoch, model, loader, population, args, saver, output_dir, loader_eval, validate_loss_fn, eval_metrics, amp_autocast=suppress):
     ...
     def score_func(population):
          ...
          score_lst = [-i/(args.de_batch_size//args.mini_batch_size) for i in acc1_all]
          return score_lst
     ...
     for de_iter in range(args.de_iters+1):
          if de_iter==0: 
               score_lst = score_func(population)
               best_score = -min(score_lst)
          else:
               population, score_lst, bestidx, de_paras = sade.evolve4(
          ...
          de_iter_acc = [round(-i.item(), 4) for i in score_lst]
          if args.local_rank == 0:
               _logger.info('de_iter:{}, best_score:{:>7.4f}, de_iter_acc: {}'.format(de_iter, -min(score_lst), de_iter_acc))
          ...
          de_iter_dict = OrderedDict([('iter', de_iter), ('train_acc', best_score.item())])
          update_summary(
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        if batch_idx > args.log_interval: break
