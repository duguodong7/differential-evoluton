...
def evolve4(cost_func,epoch,bounds,dim,popsize,population,population_f,paras,model,loader,ckp):
    ...
    for idx in range(0, popsize):
        if np.random.rand()<0.05:
            ...
            if k==0:
                ...
                x_new = population[id1] + f * (popu1lation[id2] - population[id3]) + \
    ...
    end2 = time.time()
    new_population_f = cost_func(model, pop, loader, ckp, epoch)
    ...
    for i_gen in range(250):
        pop = []
        ## prepare the child by Guassian walk around best solution
        for idx in range(0, popsize/2):
            sigma = np.absolute(np.log(i_gen)/i_gen*(population[idx] - population[bestidx]))
            pos_new = np.random.normal(loc=population[bestidx], scale=sigma, size=None) + \
                      + np.random.rand()*population[bestidx] - np.random.rand()*population[idx]
            pop.append(pos_new)
            
        new_population_f = cost_func(model, pop, loader, ckp, epoch)
        ## arrange list of child
        childidx = torch.topk(new_population_f, popsize/2, largest=False, sorted=True)
        ## check worst parent
...
if __name__ == '__main__':
    evolve4(cost_func,epoch,bounds,dim,popsize,population,population_f,paras,model,loader,ckp):
